...
import net.intellizone.coupon.city.model.IPEntry;
import net.intellizone.coupon.city.model.IPLocation;
import net.intellizone.coupon.city.model.Message;
import net.intellizone.utils.IpUtils;
 ...
 * @since city 1.0
 */
public class IPSeekService {
	// 日志记录器
	private static final Logger logger = Logger.getLogger(IPSeekService.class.getName());
	...
	public IPSeekService(String fileName, String dir) {
				...
				if (files[i].isFile()) {
					if (files[i].getName().toLowerCase().equals(filename)) {
						try {
							...
							logger.warn("IP地址信息文件没有找到，IP显示功能将无法使用", e1);
							ipFile = null;
						}
						break;
					}
		...
		if (ipFile != null) {
			try {
				...
				logger.warn("IP地址信息文件格式有错误，IP显示功能将无法使用", e);
				ipFile = null;
			}
		}
	}
